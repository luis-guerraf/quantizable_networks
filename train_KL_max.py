...
if FLAGS.model == 'models.s_resnet':
    save_filename = "_" + FLAGS.dataset + "_" + FLAGS.model + str(FLAGS.depth) + "_" + \
                    ...
                    + "_".join(str(e) for e in FLAGS.bitwidth_list) + "_KL.pt"
else:
...
def forward_loss(output, outputs_p, outputs_p_max, criterion, target, meter):
    """forward model and return loss"""
    if outputs_p is not None:
        # KL_Diverence needs probabilities not logits
        # loss = torch.mean(criterion(output, target)) + (FLAGS.lr*10)*KL_Divergence()(outputs_p_max, outputs_p)
        loss = torch.mean(criterion(output, target)) + KL_Divergence()(outputs_p_max, outputs_p)
    else:
...
def run_one_epoch(
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        if train:
            ...
            optimizer.zero_grad()
            if getattr(FLAGS, 'slimmable_training', False):
                outputs_p_max = torch.cuda.FloatTensor(FLAGS.batch_size, FLAGS.num_classes).fill_(0)
                for i, width_mult in enumerate(sorted(FLAGS.width_mult_list, reverse=True)):
                    for j, bitwidth in enumerate(sorted(FLAGS.bitwidth_list, reverse=True)):
                        ...
                        if (i == 0) and (j == 0):
                            # Compute logits
                            outputs_max = model(input)

                            # Compute output probabilities

                            outputs_p_max = F.softmax(outputs_max, dim=1).detach()

                            loss = forward_loss(
	                            outputs_max, None, None, criterion, target,
                            ...
                            loss = forward_loss(
	                            outputs, outputs_p, outputs_p_max, criterion, target,
	                            meters[str(width_mult)+str(bitwidth)])
                            loss.backward()

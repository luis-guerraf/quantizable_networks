...
from models.KL_Divergence import KL_Divergence
import torch.nn.functional as F

if FLAGS.model == 'models.s_resnet':
    save_filename = FLAGS.model + str(FLAGS.depth) + "_" + \
                    '_'.join(str(e) for e in FLAGS.width_mult_list) + "__" \
                    + "_".join(str(e) for e in FLAGS.bitwidth_list) + "_KL_ensemble.pt"
else:
    save_filename = FLAGS.model + "_" + \
                    '_'.join(str(e) for e in FLAGS.width_mult_list) + "__" \
                    + "_".join(str(e) for e in FLAGS.bitwidth_list) + "_KL_ensemble.pt"
...
def forward_loss(output, outputs_p, outputs_p_avg, criterion, target, meter):
    """forward model and return loss"""
    if outputs_p is not None:
        # KL_Diverence needs probabilities not logits
        # loss = torch.mean(criterion(output, target)) + (FLAGS.lr*10)*KL_Divergence()(outputs_p_avg, outputs_p)
        loss = torch.mean(criterion(output, target)) + KL_Divergence()(outputs_p_avg, outputs_p)
    else:
...
def run_one_epoch(
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        if train:
            ...
            optimizer.zero_grad()
            if getattr(FLAGS, 'slimmable_training', False):
                outputs = torch.cuda.FloatTensor(len(FLAGS.width_mult_list), len(FLAGS.bitwidth_list),
                                                 FLAGS.batch_size, FLAGS.num_classes).fill_(0)
                for i, width_mult in enumerate(sorted(FLAGS.width_mult_list, reverse=True)):
                    for j, bitwidth in enumerate(sorted(FLAGS.bitwidth_list, reverse=True)):
                        model.apply(lambda m: setattr(m, 'width_mult', width_mult))
                        model.apply(lambda m: setattr(m, 'bitwidth', bitwidth))

                        # Compute logits
                        outputs[i][j] = model(input)

                # Compute output probabilities
                outputs_p = F.softmax(outputs, dim=3)

                # Compute mean probability
                outputs_p_avg = torch.mean(torch.mean(outputs_p, dim=1), dim=0).detach()

                # Ensemble loss
                # NegativeLogLoss = torch.nn.NLLLoss(reduction='none').cuda()
                # loss = torch.mean(NegativeLogLoss(torch.log(outputs_p_avg), target))
                # print('Ensemble loss {:.4f}'.format(loss))

                for i, width_mult in enumerate(sorted(FLAGS.width_mult_list, reverse=True)):
                    for j, bitwidth in enumerate(sorted(FLAGS.bitwidth_list, reverse=True)):
                        loss = forward_loss(
                            outputs[i][j], outputs_p[i][j], outputs_p_avg, criterion, target,
            ...
            if getattr(FLAGS, 'slimmable_training', False):
                for width_mult in sorted(FLAGS.width_mult_list, reverse=True):
                    for bitwidth in sorted(FLAGS.bitwidth_list, reverse=True):
                        ...
                        forward_loss(
                            output, None, None, criterion, target,
                            meters[str(width_mult)+str(bitwidth)])
